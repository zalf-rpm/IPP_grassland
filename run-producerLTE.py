...
NUTS1_REGIONS = "data/germany/NUTS_RG_03M_25832.shp"
...
def run_producer(server={"server": None, "port": None}, shared_id=None):
    ...
    config = {
        ...
        "end-row": "-1",
        "path_to_dem_grid": "",
        "sim.json": "simLTE.json",
        "crop.json": "cropLTE.json",
        "site.json": "siteLTE.json",
        "setups-file": "sim_setups_LTE.csv",
        "run-setups": "[1]",
        "shared_id": shared_id
    }
        ...
        for srow in range(0, srows):
            ...
            for scol in range(0, scols):
               ...
               #     tcoords[gw_max_crs] = soil_crs_to_x_transformers[gw_max_crs].transform(sr, sh)
                ...
                if setup.get("FieldConditionModifier") is not None:
                    crop_name = env_template["cropRotation"][0]["worksteps"][0]["crop"][2]  # e.g., "RYE"
                    env_template["crops"][crop_name]["cropParams"]["species"]["FieldConditionModifier"] = float(setup["FieldConditionModifier"])
